...
**Grafico 1 — Mappa delle 34 sedi nel mondo**
Dot map globale con un punto per ogni sede: 18 in Italia (città nominate nel PDF: Bari, Bologna, Bolzano, Cagliari, Catania, Chieti, Empoli, Genova, Jesi, Milano, Napoli, Padova, Parma, Perugia, Roma, Torino, Treviso, Verona) e 16 all'estero (29 in Europa + USA, Messico, Brasile, Thailandia, India). L'Italia è mostrata in zoom con i singoli punti città; il mondo con i punti per paese. Nello step 0 compare solo la mappa italiana; nello step 1 si allarga al mondo e i punti internazionali entrano con un'animazione. Racconta l'espansione geografica in modo immediato.
...
**Grafico 1 — La struttura di governance: tre organi, una direzione**
Visualizzazione a colonne o layer sovrapposti dei tre organi principali, con i dati reali estratti dal PDF (pp. 39–41).

- **CdA** (9 membri): dot matrix 3×3, colorati per genere (blu = uomo, arancio = donna). Ogni dot ha iniziali e ruolo al rollover. Sotto, anzianità media: 14 anni.
- **Collegio Sindacale** (5 membri): dot matrix 1×5, stesso schema cromatico. Include Presidente (Fabrizio Rossi) e gli altri 4 sindaci.
- **COS — Comitato Operativo di Sostenibilità** (15 membri, p.41): dot matrix con colore per genere e label della funzione (HR, Marketing, DEI, Legal, ecc.). Il COS è più numeroso e più gender-balanced degli altri due organi — un dato che emerge visivamente.

Nello step 0 compare solo il CdA. Nello step 1 si aggiunge il Collegio Sindacale. Nello step 2 entra il COS, che per numero e composizione racconta come la governance della sostenibilità sia trasversale a tutte le funzioni aziendali.

Dati già disponibili nei JSON (`governance.json` per CdA e certificazioni; dati COS e Collegio Sindacale da integrare con i valori del PDF p.40–41).
...
**Grafico 1 — Donut chart mix energetico**
Grafico ad anello che divide i 19.869 MWh totali in due fette: fossili (69,6%, arancio) e rinnovabili (30,4%, verde). Al centro il valore totale in MWh. Nello step successivo il donut "esplode" mostrando le tre voci fossili: carburanti flotta (62%), gas riscaldamento (5,7%), elettricità fossile (2%). Nello step 2, i pallini che rappresentano la flotta auto si ridistribuiscono formando un nuovo raggruppamento — per visualizzare il peso della mobilità sul totale dei consumi.

**Grafico 2 — Arc chart elettricità rinnovabile**
Grafico a semicerchio o arco (gauge) separato dal precedente, che mostra solo la quota di elettricità da rinnovabili: 94% in verde acceso su sfondo scuro. Il numero "94%" compare con un'animazione di fill progressivo dell'arco. Comunica un dato forte in modo autonomo: l'elettricità è quasi tutta verde.

**Grafico 3 — Donut emissioni Scope 1+2 per fonte**
Grafico ad anello delle emissioni, diviso in tre voci: flotta auto (60,6%, arancio), elettricità sedi (35,4%, blu), gas naturale (4%, rosso). Etichette esterne. In calce, una nota contestuale: "Prima Convention carbon-neutral organizzata nel 2024."
...
**Grafico 1 — Dot plot delle persone per area geografica e fascia d'età**
...
**Grafico 1 — Bubble chart delle 5 categorie di iniziative**
5 cerchi disposti a pentagono, dimensionati proporzionalmente al numero di iniziative per categoria (Benessere sociale n=8, Inclusione digitale n=7, Educazione n=8, Ambiente n=4, Sport e cultura n=3). Ogni cerchio ha un colore diverso e mostra al centro sia il numero di iniziative che la parola "iniziative" (es: "8 iniziative"). Linee sottili collegano i cerchi a un centro comune. Negli step successivi, ogni cerchio si illumina e gli altri si attenuano, mentre al centro appaiono gli esempi specifici della categoria attiva.

**Grafico 2 — Big numbers tipografici**
Layout editoriale con tre numeri grandi in sequenza: "450.000 €" (valore totale investito), "30+" (iniziative), "80" (giovani all'Hackathon). Ciascun numero è accompagnato da una riga di testo descrittivo. Nessun grafico vero e proprio — è pura tipografia. Chiude la sezione con un senso di concretezza e scala.

---

## Note editoriali trasversali

- **Cap.1 — dati ora disponibili**: fatturato 875,7M€, 34 sedi (18 IT + 16 estero), 15 paesi, nomi delle 18 città italiane, 10 aree di servizio, 17 temi ESG dalla matrice di materialità. I grafici proposti si basano interamente su questi dati.
- **Cap.2 — dati per la mappa fornitori**: la distribuzione regionale è presente nel PDF (p.56) come grafico visivo con numeri. I numeri precisi per regione vanno estratti manualmente (i principali visibili: 441, 278, 191, 134, 109, 86...). Sarà necessario associarli alle regioni prima di costruire il grafico.
- **Il capitolo Persone è il più denso.** Considera se dividere la storia dei dipendenti (geografia, età, turnover) dal tema genere/inclusione, oppure tenerli insieme come fa il PDF.
- **Il capitolo Ambiente ha la narrativa più forte** perché contiene il dato sorprendente della flotta vs. l'elettricità rinnovabile. Vale la pena dargli spazio con 3 grafici distinti come proposto.
- **Il tono è sempre positivo e celebrativo** — interno aziendale. Ogni dato va letto come punto di partenza o traguardo raggiunto, mai come critica.
- **L'ordine proposto crea un arco narrativo**: dal "cosa facciamo" (innovazione) → "come ci governa" (etica) → "che impatto abbiamo" (ambiente) → "chi siamo" (persone) → "cosa restituiamo" (comunità).
- **Hero e Outro**: rimangono invariati rispetto alla bozza precedente.
